...
const BlogPostCard = ({ content, author }) => {
    ...
    let { blog_id: id, publishedAt, tags, title, des, banner, activity: { total_likes } } = content;
    ...
    return (
        <Link to={`/blog/${id}`} className="flex gap-8 items-center border-b border-grey pb-5 mb-4">
            <div className="w-full mb-7">
                ...
                <p className="my-3 text-xl font-inter leading-7 max-sm-hidden md:max[1100px]:hidden line-clamp-2">{des}</p>

                <div className="flex gap-4 items-center mt-7">
                    <span className="btn-light py-1 px-4">{tags[0]}</span>
                    <span className="ml-3 flex items-center gap-2 text-dark"><i className="fi fi-rr-heart text-xl"></i>{total_likes}</span>
                </div>
            </div>
