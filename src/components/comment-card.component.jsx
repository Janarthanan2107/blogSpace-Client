...
const CommentCard = ({ index, leftVal, commentData }) => {
    ...
    const loadReplies = ({ skip = 0, currentIndex = index }) => {
        if (commentsArr[currentIndex].children.length) {
            ...
            axios.post(domain + "/blog/comment/getReplies", { _id: commentsArr[currentIndex]._id, skip })
                .then(({ data: { replies } }) => {
    ...
    const deleteComment = (e) => {
        ...
        axios.post(domain + "/blog/comment/delete", { _id }, {
            headers: {
        ...
        <div className="w-full" style={{ paddingLeft: `${leftVal * 10}px` }}>
            <div className="my-5 p-6 rounded-md border-grey">
                ...
                <div className="flex gap-5 items-center mt-5">
                    ...
                    {
                        username == commented_by_username || username == blog_author ? <button className="p-2 px-3 rounded-md border border-grey ml-auto hover:bg-red/30 hover:text-red flex items-center" onClick={deleteComment}>
